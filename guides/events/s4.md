...
As documented in the [Service Consumption guide](../services/consuming-services#external-service-api), get, and `cds import` the API specification of an SAP S/4HANA service you want to receive events from. For example, for "BusinessPartner" using [SAP Business Accelerator Hub](https://api.sap.com/):
...
```
[Learn more about importing SAP S/4HANA service APIs.](../services/consuming-services#external-service-api){.learn-more}
...
::: tip
If using SAP Event Mesh, please see [CloudEvents Standard](messaging#cloudevents) and [Node - Messaging - CloudEvents Protocol](../../node.js/messaging#cloudevents-protocol) to learn about `format: 'cloudevents'`, `publishPrefix` and `subscribePrefix`.
:::
...
To ease the pain of the afore-mentioned topic rewriting effects, CAP has built-in support for [SAP Event Mesh](./event-mesh) as well as [SAP Cloud Application Event Hub](./event-hub).
...
How to create the necessary service instances and use them with a CAP application was already described in the previous sections [SAP Event Mesh](./event-mesh) and [SAP Cloud Application Event Hub](./event-hub), respectively.
