...
#### 1. Field help based on @Common.ValueList 

If the property marked as the organizational attribute has a `@Common.ValueList` annotated, the entity specified as the `@Common.ValueList.CollectionPath` is considered to be the source for the field help.
...
#### 2. Field help based on association target

If no value list is specified, but the property is an association, the associated entity is used for the field help.
...
#### 3. Field help based on distinct query

If the property is not an association, a field help is created by building a SELECT distinct query on that column.
...
If you have a multi-tenancy CAP application the following guide is from the perspective of a subscriber of your application. You as a provider would not configure data controllers or deletion & destruction rules.
