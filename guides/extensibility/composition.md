...
[More about Vue.js in our _Getting Started in a Nutshell_](../../get-started/bookshop#serving-uis){.learn-more} [Learn more about serving Fiori UIs.](../uis/fiori){.learn-more}
...
As a next step, following CAP's [Grow-as-you-go](../../get-started/features#grow-as-you-go) philosophy, we can run the services as separate processes to test the remote integration, but still locally in a low-complexity setup. We use the [_automatic binding by `cds watch`_](#bindings-via-cds-watch) as follows:
