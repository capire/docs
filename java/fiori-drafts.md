...
| POST with `draftPrepare` action and `$select=DraftMessages`            | `DraftService.EVENT_DRAFT_PREPARE`        | Simulates the draft activation to ensure complete validation of the document. It triggers a `draftActivate` (internally runs a `CREATE` or `UPDATE` event) in an isolated transaction, which is ultimately rolled back.          | 
