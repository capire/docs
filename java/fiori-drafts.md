...
| POST with `draftPrepare` action and `$select=DraftMessages`            | `DraftService.EVENT_CREATE`        | Creates a new active entity including all handlers and validations but ultimately performs a transaction rollback.          | 
