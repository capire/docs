...
| POST with `draftPrepare` action and `$select=DraftMessages`            | `DraftService.EVENT_DRAFT_PREPARE`        | Simulates the draft activation to validate your document completely. The system triggers a `draftActivate` operation (internally runs a `CREATE` or `UPDATE` event) in an isolated transaction and then rolls it back.         | 
