...
Each `kind` defines its own set of expected configuration properties (for example, `credentials`, `model`,`service`..). Thus allows creating services without configurations and [service bindings](#service-bindings), for example, you could connect to a local SQLite database in your tests like this:
