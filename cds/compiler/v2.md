...
When there's no unique target for [**_auto-redirection_**](../../guides/services/providing-services#redirected-associations),
...
of [_Scoped Names_](../cdl#scoped-names) and [_Managed Compositions_](../cdl#managed-compositions) also for [_Localized Data_](../../guides/uis/localized-data#behind-the-scenes).
...
[behind the scenes](../../guides/uis/localized-data#behind-the-scenes) into the database, but the CSN doesn't contain them anymore.
...
CAP Java allows to [provide initial data](../../guides/databases/initial-data) to your application using CSV files. The name of the CSV file should adhere to the pattern `<namespace>-<entity>.csv` (`<qualified-entity-name>.csv`). Renaming the CSV file is recommended.
